import {BasicColumn} from '/@/components/Table';
import {FormSchema} from '/@/components/Table';
import { rules} from '/@/utils/helper/validator';
import { render } from '/@/utils/common/renderUtils';

export const columns: BasicColumn[] = [
   //  {
   //  title: '射线装置台帐主键',
   //  dataIndex: 'recordId'
   // },
   // {
   //  title: '许可证申请单位主键',
   //  dataIndex: 'companyId'
   // },
   {
    title: '装置名称',
    dataIndex: 'setName'
   },
   {
    title: '规格型号',
    dataIndex: 'specAndModel'
   },
   {
    title: '射线装置类别 值内容：Ⅰ类 Ⅱ类 Ⅲ类',
    dataIndex: 'setType'
   },
   {
    title: '射线装置用途 ',
    dataIndex: 'useType'
   },
   {
    title: '电压',
    dataIndex: 'voltage'
   },
   {
    title: '电流',
    dataIndex: 'amperemeter'
   },
   {
    title: '功率',
    dataIndex: 'power'
   },
   {
    title: '场所',
    dataIndex: 'site'
   },
   {
    title: '来源',
    dataIndex: 'source'
   },
   {
    title: '来源审核人',
    dataIndex: 'srcAuditperson'
   },
   {
    title: '来源审核日期',
    dataIndex: 'srcAuditdate'
   },
   {
    title: '去向',
    dataIndex: 'destination'
   },
   {
    title: '去向审核人',
    dataIndex: 'destAuditperson'
   },
   {
    title: '去向审核日期',
    dataIndex: 'destAuditdate'
   },
   {
    title: '是否历史',
    dataIndex: 'beHistory'
   },
   {
    title: '粒子能量',
    dataIndex: 'particleEnergy'
   },
   {
    title: '备注',
    dataIndex: 'remark'
   },
   {
    title: '用户类型',
    dataIndex: 'userType'
   },
   {
    title: '状态',
    dataIndex: 'status'
   },
   {
    title: '添加用户',
    dataIndex: 'insertUser'
   },
   {
    title: '添加时间',
    dataIndex: 'insertDate'
   },
   {
    title: '其它用途',
    dataIndex: 'useTypeOther'
   },
   {
    title: '详细场所',
    dataIndex: 'siteDetail'
   },
   // {
   //  title: '业务id',
   //  dataIndex: 'ywid'
   // },
   // {
   //  title: '业务主键id',
   //  dataIndex: 'requeirYwid'
   // },
   {
    title: '装置名称详细',
    dataIndex: 'setNameInfo'
   },
   {
    title: '场所编码',
    dataIndex: 'siteCode'
   },
   {
    title: '标志',
    dataIndex: 'flag'
   },
   // {
   //  title: 'IS_DELETE',
   //  dataIndex: 'isDelete'
   // },
   // {
   //  title: 'UPDATETIME',
   //  dataIndex: 'updatetime'
   // },
   // {
   //  title: 'AMENDED_PATTERN',
   //  dataIndex: 'amendedPattern'
   // },
   // {
   //  title: 'UNIT_SOURCE_ID',
   //  dataIndex: 'unitSourceId'
   // },
];

export const searchFormSchema: FormSchema[] = [
 {
    label: '所属区域',
    field: 'recordId',
   component: 'JDictSelectTag',
   componentProps:{
     dictCode:"tt_zd_commoninfo,conname,concode,contype='jdjc_bhdq'",
   }
  },
 {
    label: '行政区',
    field: 'companyId',
    component: 'Input'
  },
  {
    label: '单位名称',
    field: 'companyId',
    component: 'Input'
  },
  {
    label: '许可证号',
    field: 'companyId',
    component: 'Input'
  },
  {
    label: '装置名称',
    field: 'setName',
    component: 'Input'
  },
  {
    label: '用途',
    field: 'useType',
    component: 'JDictSelectTag',
    componentProps:{
      dictCode:"tt_zd_commoninfo,conname,concode,contype like '%_USE_TYPE'",
    }
  },
  {
    label: '工作场所',
    field: 'site',
    component: 'Input'
  },
  {
    label: '装置类别',
    field: 'setType',
    component: 'JDictSelectTag',
    componentProps:{
      dictCode:"tt_zd_commoninfo,conname,concode,contype='sxzz_lb'",
    }
  },
  {
    label: '状态',
    field: 'status',
    component: 'JDictSelectTag',
    componentProps:{
      dictCode:"tt_zd_commoninfo,conname,concode,contype='device_status_dict'",
    }
  },
];

export const formSchema: FormSchema[] = [
  // TODO 主键隐藏字段，目前写死为ID
  {label: '', field: 'id', component: 'Input', show: false},
  // {
  //   label: '射线装置台帐主键',
  //   field: 'recordId',
  //   component: 'Input',
  // },
  // {
  //   label: '许可证申请单位主键',
  //   field: 'companyId',
  //   component: 'Input',
  // },
  {
    label: '装置名称',
    field: 'setName',
    component: 'Input',
  },
  {
    label: '规格型号',
    field: 'specAndModel',
    component: 'Input',
  },
  {
    label: '射线装置类别 值内容：Ⅰ类 Ⅱ类 Ⅲ类',
    field: 'setType',
    component: 'Input',
  },
  {
    label: '射线装置用途 ',
    field: 'useType',
    component: 'Input',
  },
  {
    label: '电压',
    field: 'voltage',
    component: 'Input',
  },
  {
    label: '电流',
    field: 'amperemeter',
    component: 'Input',
  },
  {
    label: '功率',
    field: 'power',
    component: 'Input',
  },
  {
    label: '场所',
    field: 'site',
    component: 'Input',
  },
  {
    label: '来源',
    field: 'source',
    component: 'Input',
  },
  {
    label: '来源审核人',
    field: 'srcAuditperson',
    component: 'Input',
  },
  {
    label: '来源审核日期',
    field: 'srcAuditdate',
    component: 'DatePicker'
  },
  {
    label: '去向',
    field: 'destination',
    component: 'Input',
  },
  {
    label: '去向审核人',
    field: 'destAuditperson',
    component: 'Input',
  },
  {
    label: '去向审核日期',
    field: 'destAuditdate',
    component: 'DatePicker'
  },
  {
    label: '是否历史',
    field: 'beHistory',
    component: 'InputNumber',
  },
  {
    label: '粒子能量',
    field: 'particleEnergy',
    component: 'Input',
  },
  {
    label: '备注',
    field: 'remark',
    component: 'Input',
  },
  {
    label: '用户类型',
    field: 'userType',
    component: 'InputNumber',
  },
  {
    label: '状态',
    field: 'status',
    component: 'Input',
  },
  {
    label: '添加用户',
    field: 'insertUser',
    component: 'Input',
  },
  {
    label: '添加时间',
    field: 'insertDate',
    component: 'DatePicker'
  },
  {
    label: '其它用途',
    field: 'useTypeOther',
    component: 'Input',
  },
  {
    label: '详细场所',
    field: 'siteDetail',
    component: 'Input',
  },
  // {
  //   label: '业务id',
  //   field: 'ywid',
  //   component: 'Input',
  // },
  // {
  //   label: '业务主键id',
  //   field: 'requeirYwid',
  //   component: 'Input',
  // },
  {
    label: '装置名称详细',
    field: 'setNameInfo',
    component: 'Input',
  },
  {
    label: '场所编码',
    field: 'siteCode',
    component: 'Input',
  },
  {
    label: '标志',
    field: 'flag',
    component: 'Input',
  },
  // {
  //   label: 'IS_DELETE',
  //   field: 'isDelete',
  //   component: 'InputNumber',
  // },
  // {
  //   label: 'UPDATETIME',
  //   field: 'updatetime',
  //   component: 'DatePicker'
  // },
  // {
  //   label: 'AMENDED_PATTERN',
  //   field: 'amendedPattern',
  //   component: 'Input',
  // },
  // {
  //   label: 'UNIT_SOURCE_ID',
  //   field: 'unitSourceId',
  //   component: 'Input',
  // },
];
